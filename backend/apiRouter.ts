...
import { analyzeContext } from './pipeline/contextAnalyzer.js';
import { detectIntent } from './pipeline/intentDetector.js';
import { buildWorkflowPlan } from './pipeline/planner.js';
import { executeWorkflowPlan } from './pipeline/executor.js';
import { buildFinalPrompt } from './pipeline/promptBuilder.js';
import { streamModelResponse } from './pipeline/provider.js';
import { memoryManager } from './memory/memoryManager.js';
import { toolRegistry } from './tools/toolRegistry.js';
import { agentOrchestrator } from './agents/agentOrchestrator.js';
import { historyManager } from './services/historyManager.js';
import { Message, PipelineEvent } from './types.js';

export function createApiRouter(): express.Router {
  const router = express.Router();

  // Health check endpoint (support both /health and /api/health for Netlify serverless routing flexibility)
  router.get(['/health', '/api/health'], (req, res) => {
    res.json({
      status: 'ok',
      system: 'AI Radar OS',
      version: '1.0.0',
      mode: 'SERVER_OR_NETLIFY_FUNCTION',
  ...
  router.post(['/sessions', '/api/sessions'], (req, res) => {
    const { title } = req.body;
    const session = historyManager.createSession(title || 'Nova Conversa');
    res.json(session);
  });

  router.patch(['/sessions/:id', '/api/sessions/:id'], (req, res) => {
    const { id } = req.params;
  ...
  router.get(['/sessions/:id/messages', '/api/sessions/:id/messages'], (req, res) => {
    const { id } = req.params;
    res.json(historyManager.getMessages(id));
  });

  router.delete(['/sessions/:sessionId/messages/:messageId', '/api/sessions/:sessionId/messages/:messageId'], (req, res) => {
  ...
  // Tools & Agents API
  router.get(['/tools', '/api/tools'], (req, res) => {
    res.json(toolRegistry.getAllTools());
  });

  router.get(['/agents', '/api/agents'], (req, res) => {
    res.json(agentOrchestrator.getAllAgents());
  });

  // Memory Inspector API
  router.get(['/memory', '/api/memory'], (req, res) => {
    res.json(memoryManager.getFullState());
  });

  router.post(['/memory/clear', '/api/memory/clear'], (req, res) => {
    memoryManager.clearAll();
    res.json({ success: true, message: 'Memory cleared.' });
  });

  // File Upload to Knowledge Base
  router.post(['/upload', '/api/upload'], (req, res) => {
  ...
  router.post(['/chat', '/api/chat', '/chat/stream', '/api/chat/stream'], chatStreamHandler);
